...
const tooltipStyle = {
  contentStyle: { backgroundColor: "#141414", border: "1px solid #1e1e1e", borderRadius: "12px", color: "#E8E6E3", boxShadow: "0 8px 32px rgba(0,0,0,0.5)" },
  itemStyle: { color: "#E8E6E3" },
};

/* ── COD vs TF Pie ── */
const COLORS_COD_TF = ["#F5A623", "#5ab468"];
  ...
  return (
    <div className="glass p-5">
      ...
      <div className="flex items-center gap-4 mb-3">
        ...
        <span className="flex items-center gap-1.5 text-[10px] text-[#6B6B78]"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: "#5ab468" }} />TF</span>
      </div>
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data} margin={{ top: 20, bottom: 30 }}>
          ...
          <Bar dataKey="tf" stackId="a" fill="#5ab468" radius={[4, 4, 0, 0]} label={<StackedBarLabel x={0} y={0} width={0} value={0} />}>
            {data.map((entry, i) => (
              <Cell key={i} />
            ))}
          </Bar>
...
function OrderDot({ cx, cy, index, dataLength }: { cx: number; cy: number; index: number; dataLength: number }) {
  ...
  return (
    <g>
      {isToday && <circle cx={cx} cy={cy} r={10} fill="#5ab468" opacity={0.15} />}
      <circle cx={cx} cy={cy} r={isToday ? 5 : 3} fill="#5ab468" stroke={isToday ? "#E8E6E3" : "rgba(90,180,104,0.4)"} strokeWidth={isToday ? 2.5 : 1} />
    </g>
  );
}
...
export function TrendLineChart({ data, totalSales, totalOrders, avgOrderValue }: TrendProps) {
  ...
  return (
    <div className="glass p-5 col-span-1 md:col-span-2 lg:col-span-3" style={{ borderColor: "rgba(245,166,35,0.08)" }}>
      {/* Header + Summary */}
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-5 gap-4">
        ...
        <div className="flex gap-5 flex-wrap">
          ...
          <div className="text-right">
            ...
            <p className="text-lg font-bold text-[#5ab468] leading-tight mt-0.5">{formatNumber(totalOrders)}</p>
          </div>
          <div className="text-right">
      ...
      <div className="flex items-center gap-5 mb-4">
        ...
        <span className="flex items-center gap-2 text-[11px] text-[#9B9BA8] font-medium">
          <span className="w-6 h-[3px] rounded-full" style={{ background: "#5ab468" }} />
          Orders
        </span>
      </div>
        ...
        <ComposedChart data={enriched} margin={{ top: 10, right: 8, bottom: 0, left: -4 }}>
          <defs>
            ...
            <linearGradient id="ordersAreaGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#5ab468" stopOpacity={0.2} />
              <stop offset="100%" stopColor="#5ab468" stopOpacity={0} />
            </linearGradient>
          </defs>
          ...
          <YAxis
            yAxisId="right"
            orientation="right"
            tick={{ fill: "#5ab468", fontSize: 10, fontWeight: 500 }}
            tickLine={false}
            axisLine={false}
            allowDecimals={false}
            label={{ value: "Orders", angle: 90, position: "insideRight", offset: 10, style: { fill: "#5ab468", fontSize: 9, fontWeight: 500 } }}
          />
          ...
          <Line
            ...
            activeDot={{ r: 6, fill: "#5ab468", stroke: "#E8E6E3", strokeWidth: 2 }}
          />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
